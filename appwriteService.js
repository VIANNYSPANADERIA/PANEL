...
function rowToObj(table, row) {
  ...
  switch (table) {
    ...
    case 'insumos':
      return {
        id, nombre: row.nombre || '', categoria: row.categoria || '',
        unidadCompra: row.unidadCompra || 'kg',
        cantidadComprada: _num(row.cantidadComprada), precioCompra: _num(row.precioCompra),
        precioBase: _num(row.precioBase), proveedor: row.proveedor || '',
        fechaCompra: row.fechaCompra || '', observaciones: row.observaciones || '',
        historial: _p(row.historial, []) || []
      };
    case 'recetas':
      return {
        id, productoId: row.productoId || '',
        ingredientes: _p(row.ingredientes, []) || [],
        panesPorLata: _num(row.panesPorLata, 1) || 1,
        pesoPorPan: _num(row.pesoPorPan)
      };
    case 'produccion':
      return {
        id, fecha: row.fecha || '', productoId: row.productoId || '',
        latas: _num(row.latas), panesPorLata: _num(row.panesPorLata, 1) || 1,
        pesoPorPan: _num(row.pesoPorPan), costoLata: _num(row.costoLata)
      };
    case 'costosIndirectos':
      return {
        id, nombre: row.nombre || '', categoria: row.categoria || 'Otros',
        valor: _num(row.valor), periodicidad: row.periodicidad || 'mensual',
        fechaInicio: row.fechaInicio || '', activo: row.activo === undefined ? true : !!row.activo,
  ...
  switch (table) {
    case 'clientes':
      return {
        ...
        lat: _nul(o.lat), lng: _nul(o.lng), orden: _num(o.orden),
        saldoInicial: _nul(o.saldoInicial), tipoSaldo: _oNull(o.tipoSaldo)
      };
    case 'productos':
      return {
        ...
        clienteExclusivo: _oNull(o.clienteExclusivo)
      };
    case 'pedidos':
      return {
    ...
    case 'insumos':
      return {
        nombre: o.nombre || '', categoria: o.categoria || '',
        unidadCompra: o.unidadCompra || 'kg',
        cantidadComprada: _num(o.cantidadComprada), precioCompra: _num(o.precioCompra),
        precioBase: _num(o.precioBase), proveedor: o.proveedor || '',
        fechaCompra: o.fechaCompra || '', observaciones: o.observaciones || '',
        historial: _j(o.historial || [])
      };
    case 'recetas':
      return {
        productoId: o.productoId || '', ingredientes: _j(o.ingredientes || []),
        panesPorLata: Math.max(1, Math.round(_num(o.panesPorLata, 1))),
        pesoPorPan: _num(o.pesoPorPan)
      };
    case 'produccion':
      return {
        fecha: o.fecha || '', productoId: o.productoId || '', latas: _num(o.latas),
        panesPorLata: Math.max(1, Math.round(_num(o.panesPorLata, 1))),
        pesoPorPan: _num(o.pesoPorPan), costoLata: _num(o.costoLata)
      };
    case 'costosIndirectos':
      return {
        nombre: o.nombre || '', categoria: o.categoria || 'Otros', valor: _num(o.valor),
        periodicidad: o.periodicidad || 'mensual', fechaInicio: o.fechaInicio || '',
        activo: o.activo !== false, observaciones: o.observaciones || ''
      };
...
async function awCargarTodo(opts = {}) {
  ...
  const claves = ['clientes', 'productos', 'pedidos', 'recaudo', 'cartera', 'empleados',
    ...
    'insumos', 'recetas', 'produccion', 'costosIndirectos'];
...
async function awSesion() {
  try { return await _sdk.account.get(); } catch { return null; }
}
...
async function awLogin(email, password) {
  try {
    await _sdk.account.createEmailPasswordSession({ email, password });
  } catch (e) {
    const code = Number(e?.code ?? 0);
    if (code === 401) throw new PermissionError('Correo o contraseña incorrectos.', { code, op: 'login', cause: e });
    throw _normalize(e, 'login');
  }
  const user = await awSesion();
...
async function awLogout() {
  ...
  try { await _sdk.account.deleteSession({ sessionId: 'current' }); } catch { /* noop */ }
  location.reload();
}
