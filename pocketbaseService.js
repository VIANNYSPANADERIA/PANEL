...
/** Escrituras que aún no terminan: rowId → cuántas hay en curso. */
const _enVuelo = new Map();
/** Última vez que una escritura propia terminó (o empezó): rowId → timestamp. */
const _ultimaLocal = new Map();
...
function rowToObj(table, row) {
  ...
  obj._pbId = row.id;
  // Fecha de última modificación en el servidor: sirve para elegir la fila
  // correcta si un mismo pedido quedó con dos filas de recaudo.
  if (row.updated) obj._upd = row.updated;
...
/**
 * Envuelve una escritura para saber que la fila tiene cambios locales que el
 * servidor todavía no confirma. Mientras tanto, ni la resincronización ni un
 * evento Realtime pueden reemplazar esa fila con una versión vieja.
 */
async function _conEscritura(rowId, fn) {
  if (!rowId) return fn();
  _enVuelo.set(rowId, (_enVuelo.get(rowId) || 0) + 1);
  _ultimaLocal.set(rowId, Date.now());
  try { return await fn(); }
  finally {
    const n = (_enVuelo.get(rowId) || 1) - 1;
    if (n > 0) _enVuelo.set(rowId, n); else _enVuelo.delete(rowId);
    _ultimaLocal.set(rowId, Date.now());
  }
}

/**
 * ¿La fila tiene cambios locales más nuevos que lo que el servidor pudo
 * devolver? `desde` = momento en que empezó la descarga (0 para eventos).
 */
function _cambioLocalPendiente(table, rowId, desde = 0) {
  if (!rowId) return false;
  if (_enVuelo.has(rowId)) return true;
  if (desde) {
    const t = _ultimaLocal.get(rowId);
    if (t && t >= desde) return true;
  }
  // Cola de guardado del recaudo (vive en index.html): cambios aún sin enviar.
  if (table === 'recaudo' && typeof _recaudoCola !== 'undefined' && _recaudoCola.has(rowId)) return true;
  return false;
}
...
async function actualizar(table, rowId, data) {
  return _conEscritura(rowId, async () => {
    _marcarPropia(rowId);
...
/**
 * Une la lista del servidor con la local respetando lo que aún no se confirmó:
 * una fila con escritura pendiente conserva su versión local (o sigue borrada).
 */
function _mezclarPendientes(table, remotos, locales, inicio) {
  const loc = new Map((locales || []).map((x) => [x.id, x]));
  const out = [];
  const vistos = new Set();
  for (const r of remotos) {
    vistos.add(r.id);
    if (_cambioLocalPendiente(table, r.id, inicio)) {
      if (loc.has(r.id)) out.push(loc.get(r.id));   // si no está local, se borró aquí
    } else out.push(r);
  }
  for (const [id, x] of loc) {
    if (!vistos.has(id) && _cambioLocalPendiente(table, id, inicio)) out.push(x);   // recién creada
  }
  return out;
}
...
async function awCargarTodo(opts = {}) {
  ...
  _bootErrors = [];
  // Todo lo que se escriba localmente desde este instante es más nuevo que lo
  // que va a devolver el servidor: esas filas no se pisan al aplicar la descarga.
  ...
  const claves = ['clientes', 'productos', 'pedidos', 'recaudo', 'cartera', 'empleados',
    ...
    'flujoConfig', 'proveedoresDeuda', 'abonosDeuda'];

  const resultados = await Promise.all([
    ...claves.map((k) => PB_OPCIONALES.has(k)
      ? listar(k, { force: true }).catch(() => null)   // aún no creada: no es error
      : _tolerante(k, listar(k, { force: true }))),
  ...
  if (d.recaudo) {
    const previo = DB.recaudo || {};
    const nuevo = {};
    for (const r of d.recaudo) {
      const dia = (nuevo[r.fecha] ??= {});
      const ya = dia[r.pedidoId];
      // Si un pedido quedó con dos filas en el servidor, gana la más reciente
      // (antes ganaba la última de la lista, que podía ser la vieja sin marcar).
      if (!ya || String(r._upd || '') > String(ya._upd || '')) dia[r.pedidoId] = r;
    }
    // Filas con cambios locales que el servidor aún no tiene: gana la local.
    // Sin esto, marcar "Recibido" y resincronizar antes de que se guardara
    // devolvía la fila sin marcar, y el siguiente guardado (p. ej. "Quitar
    // Recaudo") dejaba ese desmarcado grabado en el servidor.
    for (const f of Object.keys(previo)) {
      for (const pid of Object.keys(previo[f] || {})) {
        const loc = previo[f][pid];
        if (loc && _cambioLocalPendiente('recaudo', loc.id, inicio)) (nuevo[f] ??= {})[pid] = loc;
      }
    }
    DB.recaudo = nuevo;
  }
  if (d.flujoConfig && d.flujoConfig.length &&
      !_cambioLocalPendiente('flujoConfig', DB.costos.flujoConfig?.id, inicio)) {
    DB.costos.flujoConfig = d.flujoConfig[0];
  }
  if (d.proveedoresDeuda) DB.costos.proveedoresDeuda = _mezclarPendientes('proveedoresDeuda', d.proveedoresDeuda, DB.costos.proveedoresDeuda, inicio);
  if (d.abonosDeuda) DB.costos.abonosDeuda = _mezclarPendientes('abonosDeuda', d.abonosDeuda, DB.costos.abonosDeuda, inicio);
...
function _aplicarEvento(table, ev) {
  ...
  // Hay un cambio local de esta fila sin confirmar: el evento trae una versión
  // anterior y no debe reemplazarla (el guardado en curso la dejará al día).
  if (accion !== 'delete' && _cambioLocalPendiente(table, id)) { _log('evento viejo ignorado', table, id); return; }
  ...
  switch (table) {
    ...
    case 'recaudo': {
      ...
      if (!f || !pid) break;
      const ya = DB.recaudo[f]?.[pid];
      if (accion === 'delete') {
        // Solo se borra si es la MISMA fila (no un duplicado del mismo pedido).
        if (ya && ya.id === id) delete DB.recaudo[f][pid];
      } else {
        // Un duplicado del mismo pedido no reemplaza a la fila con cambios locales.
        if (ya && ya.id !== id && _cambioLocalPendiente('recaudo', ya.id)) break;
        (DB.recaudo[f] ??= {})[pid] = obj;
      }
      break;
    }
    case 'flujoConfig':
      if (obj && accion !== 'delete') DB.costos.flujoConfig = obj;
      break;
    case 'proveedoresDeuda': _upsertArr(DB.costos.proveedoresDeuda ??= [], id, obj, accion); break;
    case 'abonosDeuda': _upsertArr(DB.costos.abonosDeuda ??= [], id, obj, accion); break;
    case 'facturaConfig':
...
async function awIniciarRealtime(onChange) {
  ...
  try {
    ...
    for (const [logico, col] of Object.entries(PB_COLLECTIONS)) {
      const suscribir = () => _pb.collection(col).subscribe('*', (ev) => {
        try { _aplicarEvento(logico, ev); }
        catch (e) { console.error(new RealtimeError('Evento no aplicable', { cause: e })); }
      });
      // Una colección opcional que aún no existe no debe tumbar el canal en vivo.
      if (PB_OPCIONALES.has(logico)) {
        try { _rtSub.push(await suscribir()); } catch (e) { _log('sin realtime para', col); }
      } else {
        _rtSub.push(await suscribir());
      }
    }
...
/**
 * Como _persistir, pero para colecciones opcionales: si la colección aún no
 * existe en PocketBase, el dato queda en la copia local y se avisa UNA vez
 * qué colección hay que crear (en vez de fallar en cada guardado).
 */
const _avisoFaltante = new Set();
async function _persistirOpcional(table, fn, mensajeError) {
  try { return await fn(); }
  catch (e) {
    const err = _normalize(e, 'guardar');
    if (err instanceof NotFoundError) {
      if (!_avisoFaltante.has(table)) {
        _avisoFaltante.add(table);
        _notify(`Falta crear la colección "${PB_COLLECTIONS[table]}" en PocketBase. ` +
          'Mientras tanto se guarda solo en este navegador.', 'error');
      }
      if (typeof window.saveDB === 'function') window.saveDB();
      return null;
    }
...
const appwriteService = {
  ...
  /** Parámetros de la calculadora (fila única global, igual que la factura). */
  async guardarFlujoConfig(patch) {
    const cfg = Object.assign({}, DB.costos.flujoConfig || {}, patch);
    cfg.id = cfg.id || 'flujo_config_global';
    DB.costos.flujoConfig = cfg;
    await _persistirOpcional('flujoConfig', () => guardar('flujoConfig', cfg.id, cfg),
      'No se pudo guardar la configuración del flujo');
    return cfg;
  },

  async guardarProveedorDeuda(data) {
    if (!data?.id && !data?.nombre) throw new ValidationError('El proveedor necesita un nombre.');
    const lista = (DB.costos.proveedoresDeuda ??= []);
    const actual = data.id ? lista.find((x) => x.id === data.id) : null;
    if (actual) {
      Object.assign(actual, data);
      await _persistirOpcional('proveedoresDeuda', () => actualizar('proveedoresDeuda', actual.id, actual),
        'No se pudo actualizar el proveedor');
      return actual;
    }
    const obj = { id: _genId('PRV'), deudaInicial: 0, periodicidad: 'quincenal', pedidoHabitual: 0, ...data };
    lista.push(obj);
    await _persistirOpcional('proveedoresDeuda', () => crear('proveedoresDeuda', obj, obj.id),
      'No se pudo crear el proveedor');
    return obj;
  },

  async eliminarProveedorDeuda(id) {
    DB.costos.proveedoresDeuda = (DB.costos.proveedoresDeuda || []).filter((x) => x.id !== id);
    const abonos = (DB.costos.abonosDeuda || []).filter((a) => a.proveedorId === id);
    DB.costos.abonosDeuda = (DB.costos.abonosDeuda || []).filter((a) => a.proveedorId !== id);
    await _persistirOpcional('proveedoresDeuda', async () => {
      await eliminar('proveedoresDeuda', id);
      for (const a of abonos) await eliminar('abonosDeuda', a.id);
    }, 'No se pudo eliminar el proveedor');
  },

  /** Abono a un proveedor. `tipo`: 'abono' (resta deuda) | 'credito' (suma deuda). */
  async guardarAbonoDeuda(data) {
    if (!data?.proveedorId) throw new ValidationError('El abono necesita un proveedor.');
    const obj = { id: _genId('ABD'), fecha: new Date().toISOString().slice(0, 10), tipo: 'abono', valor: 0, nota: '', ...data };
    (DB.costos.abonosDeuda ??= []).push(obj);
    await _persistirOpcional('abonosDeuda', () => crear('abonosDeuda', obj, obj.id),
      'No se pudo registrar el abono');
    return obj;
  },

  async eliminarAbonoDeuda(id) {
    DB.costos.abonosDeuda = (DB.costos.abonosDeuda || []).filter((x) => x.id !== id);
    await _persistirOpcional('abonosDeuda', () => eliminar('abonosDeuda', id),
